...
class Trainer:
    ...
    def _generate_adversarial_examples(self, epoch):
        def read_from_csv(filename):
            data = []
            if os.path.exists(filename):  # 检查文件路径是否存在
                with open(filename, mode='r', encoding="MacRoman", errors='ignore') as file:
                    reader = csv.reader(file)
                    for row in reader:
                        # 处理每一行数据，转换为列表中的元组
                        inputs = (row[0],)
                        label = int(row[1])
                        # 根据 label 决定 example 类型
                        if row[2] == "Successful":
                            example_type = "adversarial_example"
                        else:
                            example_type = "nat_example"
                        data.append((inputs, label, example_type))
            return data

        def generate_adversarial_examples(epoch, num_train_adv_examples):
            file_path = '/root/autodl-tmp/TextAttack-A2T-main/adv_exm_' + str(epoch) + '.csv'
            if os.path.exists(file_path):
                adversarial_examples = read_from_csv(file_path)

                if num_train_adv_examples >= 0:
                    num_to_select = int(num_train_adv_examples)  # 计算要选择的数量
                    selected_examples = random.sample(adversarial_examples, num_to_select)  # 随机抽取指定数量的样本
        ...
        if os.path.exists(file_path):
            if num_train_adv_examples >= 0:
                adversarial_examples = generate_adversarial_examples(epoch, num_train_adv_examples)
            else:
                adversarial_examples = read_from_csv(file_path)
        else:
            if num_train_adv_examples >= 0:
    ...
    def get_train_dataloader(self, dataset, adv_dataset, batch_size):
        ...
        if adv_dataset != None:
            if num_samples != len(adv_dataset):
                print("长度不对")
                quit()
            adv_dataset = [adv_dataset[i] for i in index_list]
        dataset = [dataset[i] for i in index_list]
